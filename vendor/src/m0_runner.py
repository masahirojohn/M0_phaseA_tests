...
def _last_t_ms(json_path: str) -> int:
    """Timeline(JSON list)の末尾 t_ms を返す。失敗時は 0。"""
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            arr = json.load(f)
        if isinstance(arr, list) and arr:
            last = arr[-1].get("t_ms", arr[-1].get("t", 0))
            return int(last)
    except Exception:
        pass
    return 0
...
def main():
    ...
    cfg_duration = cfg["video"]["duration_s"]  # auto対応のため、ここではまだint化しない
    ...
    def merged_value(t_ms: int) -> Dict[str, Any]:
        ...
        vals.update(p)
        vals.update(e)
        # mouth欠落時のデフォルト（全Fallback防止）
        if "mouth" not in vals or not vals["mouth"]:
            vals["mouth"] = "closed"
        return vals

    # duration_s の決定（auto→末尾t_msから算出）
    if isinstance(cfg_duration, str) and cfg_duration.lower() == "auto":
        last_ms = max(_last_t_ms(pose_json), _last_t_ms(mouth_json), _last_t_ms(expr_json))
        duration_s = max(1, (last_ms + 999) // 1000)  # 端数切り上げ
    else:
        duration_s = int(cfg_duration)
